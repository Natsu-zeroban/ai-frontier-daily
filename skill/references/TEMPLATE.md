...
5. 发布(按爆炸半径)：**纯新增**内容(新期/新文章/要闻茶点)校验通过后直接
   ...
   **改到往期已发布内容**才先给用户预览、等确认再推。
6. **push 后先轮询线上、确认新内容已部署，再打开浏览器**(用新期号如 `vol.0N` 当探测标识，
   每 10 秒 curl 一次、最多约 5 分钟)；没等到就别开，告知用户稍后刷新。
   **打开时必须带一次性参数**(`?v=时间戳`)：Pages 的 max-age=600 会让浏览器缓存旧页 10 分钟，
   curl 探测通过 ≠ 浏览器能看到新版。
